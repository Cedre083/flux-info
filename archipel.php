...
$pageTitle = 'Comprendre l’Archipel | Relations et Interdépendances | Flux Info';
$pageDesc = 'Archipel révèle les relations entre les grands territoires de Flux Info et aide à penser les interdépendances du réel.';
...
<style>
        ...
        /* ============================================= */
        body {
            /* Identité chromatique Archipel préservée */
            background: #04000a;
            background: radial-gradient(circle at 50% 20%, #1a0033 0%, #04000a 100%);
            overflow-x: hidden;
        }
        ...
        /* Pulsation de synthèse subtile */
        .archipel-halo {
            position: fixed;
            inset: 0;
            background: radial-gradient(circle at 50% 40%, rgba(180, 100, 255, 0.08), transparent 70%);
            mix-blend-mode: screen;
            animation: archipelPulse 15s ease-in-out infinite;
            pointer-events: none;
            z-index: 1;
        }
        @keyframes archipelPulse { 0%, 100% { opacity: 0.2; } 50% { opacity: 0.4; } }

        /* ============================================= */
        /* CONTENEUR & TYPOGRAPHIE (Standard 23px)       */
        /* ============================================= */
        .container {
            max-width: 1000px; /* Plus large pour la vision d'ensemble */
            margin: 0 auto; 
        ...
        h1 {
            ...
            animation: archipelRise 1.4s cubic-bezier(0.22, 1, 0.36, 1);
            margin-top: 0;
        }

        @keyframes archipelRise {
...
<?= $headerNavigation ?>
<canvas id="archipelCanvas" aria-hidden="true"></canvas>
<div class="archipel-halo"></div>


<main class="container">
    <h1>Comprendre l’Archipel</h1>

    <p class="intro-text">
        Relier les mondes, relier les vivants.<br>
        Une invitation à penser en liens plutôt qu'en blocs isolés pour percevoir l'unité du Vivant.
    </p>

    <h2>Ce que vous allez Explorer</h2>

    <ul class="explorer-list">
        <li>- Les relations plutôt que les catégories isolées</li>
        <li>- Le signal, le milieu et les conditions qui lui donnent sens</li>
        <li>- Les dépendances réciproques au sein du vivant</li>
        <li>- Une pensée systémique capable de changer d’échelle</li>
    </ul>

    <section class="article-invitation" aria-labelledby="article-invitation-archipel">
        <p class="article-invitation__meta"><span>Article augmenté</span><span>10 min de lecture</span></p>
        <h3 id="article-invitation-archipel">La Conscience Archipélique</h3>
        <p class="article-invitation__question">Que devient notre compréhension du monde lorsque nous cessons d’isoler les phénomènes et commençons à suivre leurs relations ?</p>
        <ul class="article-invitation__promise">
            <li>Pourquoi une information n’a de sens qu’inscrite dans un milieu et des relations</li>
            <li>Comment pollinisateurs, plantes, sols et sociétés composent des réseaux de dépendance</li>
            <li>Comment distinguer fait scientifique, métaphore et réflexion quand on pense les interdépendances</li>
        </ul>
        <a href="article-archipel-conscience.php" class="btn-main">Entrer dans la Conscience Archipélique →</a>
        <p class="article-invitation__hint">Comprendre · Relier · Manipuler · Vérifier les sources</p>
    </section>
    
    <a href="index.php" style="display:block; margin-top:30px; color:#c8a0ff; text-decoration:none; font-size:0.8rem; text-transform:uppercase; opacity:0.6;">← Retour à l’Accueil</a>
</main>

<script>
/* ============================================= */
/* FOND GÉNÉRATIF: LE RÉSEAU DES MONDES         */
/* ============================================= */
const canvas = document.getElementById('archipelCanvas');
...
for (let i = 0; i < 70; i++) points.push(new ArchipelPoint());
...
function animate() {
    ...
    points.forEach((p1, i) => {
        ...
        for (let j = i + 1; j < points.length; j++) {
            ...
            if (d < 180) {
                ctx.strokeStyle = `rgba(200, 160, 255, ${0.12 * (1 - d/180)})`;
                ctx.lineWidth = 0.5;
                ctx.beginPath();
